import {errorsFormData, formData} from "~/pages/auth/login/composables/data.js";
...
export const isPasswordValid = (value, errorData) => {
    ...
    if (value.length < 6) {
        errorData.errors = ["Field Email min length should be not less than 6"]
        return false;
    }

    // Max length should be not more than 20
    if (value.length > 20) {
        errorData.errors = ["Field Email should be not more than 20"]
        return false;
    }
...
export const isFormValid = (errorsFormData) => {
    ...
    return Object.values(errorsFormData).every(field => field.errors.length === 0);
}
